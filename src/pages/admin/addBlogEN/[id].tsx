...
function Index(this: any) {
  ...
  const { data: getSpecdataEN, isLoading: isLoadinggetSpecItemsEN } =
    api.blog.getSpecBlogEN.useQuery({ id: router.query.id as any });
  ...
  const {
    mutateAsync: mutateENEdit,
    data: dataENEdit,
    isLoading: isLoadingItemsENEdit,
  } = api.blog.EditBlogEN.useMutation();
  ...
  useEffect(() => {
    setTitle(getSpecdataEN?.title as any);
    setAuthor(getSpecdataEN?.author as any);
    setText(getSpecdataEN?.text as any);
    setDate(getSpecdataEN?.date as any);
    setImage(getSpecdataEN?.image as any);
  }, [getSpecdataEN]);
